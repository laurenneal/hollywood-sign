...
export function drawLot(g, scene, p, opts = {}) {
  ...
  // Static lot props (not data counts): C-stands / apple boxes / cable runs — CLOSE only.
  // Kept sparse so the apron reads clearly and doesn't compete with the data-driven crews.
  if (close) {
    ...
    cone(g, D.stages.x + 780, A.stageRows[1].y + A.stageSize.h + 20);
  ...
  // Stars only when close enough to read — empty walk reads as pavement otherwise
  if (close) {
    g.fill(C.stuccoDk);
    for (let i = 0; i < 9; i++) {
...
function stageProp(g, x, y) {
  g.fill(26, 32, 34, 36); g.ellipse(x + 4, y + 34, 68, 18);
  g.fill(C.inkSoft);
...
function cone(g, x, y) {
  g.fill(26, 32, 34, 30); g.ellipse(x + 1, y + 6, 38, 12);
  g.fill(C.lamp);
