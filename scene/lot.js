...
export function drawLot(g, scene, p, opts = {}) {
  ...
  // decorative manhole covers (not data)
  g.fill(C.asphaltDk);
  ...
  // Static lot props (not data counts): C-stands / apple boxes / cable runs
  stageProp(g, D.stages.x + 40, A.stageRows[0].y + A.stageSize.h + 8);
  stageProp(g, D.stages.x + 520, A.stageRows[0].y + A.stageSize.h + 10);
  stageProp(g, D.stages.x + 900, A.stageRows[1].y + A.stageSize.h + 6);
  stageProp(g, D.stages.x + 280, A.stageRows[1].y + A.stageSize.h + 8);
  stageProp(g, D.stages.x + 700, A.stageRows[0].y + A.stageSize.h + 12);
  stageProp(g, D.stages.x + 140, A.stageRows[1].y + A.stageSize.h + 10);
  stageProp(g, D.stages.x + 820, A.stageRows[0].y + A.stageSize.h + 8);
  cableRun(g, D.stages.x + 160, A.stageRows[0].y + A.stageSize.h + 16);
  cableRun(g, D.stages.x + 700, A.stageRows[1].y + A.stageSize.h + 14);
  cableRun(g, D.stages.x + 420, A.stageRows[1].y + A.stageSize.h + 18);
  cableRun(g, D.stages.x + 560, A.stageRows[0].y + A.stageSize.h + 20);
  // traffic cones on aprons (decorative grip language — not data)
  cone(g, D.stages.x + 200, A.stageRows[0].y + A.stageSize.h + 18);
  cone(g, D.stages.x + 780, A.stageRows[0].y + A.stageSize.h + 20);
  cone(g, D.stages.x + 360, A.stageRows[1].y + A.stageSize.h + 16);
  cone(g, D.stages.x + 860, A.stageRows[1].y + A.stageSize.h + 18);
  cone(g, D.stages.x + 480, A.stageRows[0].y + A.stageSize.h + 22);
  cone(g, D.stages.x + 640, A.stageRows[1].y + A.stageSize.h + 20);
  plaque(g, D.stages.x + 16, D.stages.y + 6, 'SOUNDSTAGE ROW');
  ...
  // worn service path from Soundstage Row down toward Backlot Street
  g.fill(mixHex(C.ground, C.asphalt, 0.4));
  g.beginShape();
  g.vertex(D.stages.x + 170, A.stageRows[1].y + A.stageSize.h + 18);
  g.bezierVertex(D.stages.x + 220, 880, 900, 900, A.sidewalk.x + 40, A.sidewalk.y);
  g.bezierVertex(900, 920, D.stages.x + 240, 900, D.stages.x + 190, A.stageRows[1].y + A.stageSize.h + 40);
  g.endShape(g.CLOSE);

  // ---- Backlot Street
  const u = A.unionHall;
  volume(g, matStucco, u.x, u.y, u.w, u.h, C.stucco, C.roof, { awning: true, awningColor: C.lamp });
  g.fill(C.ink); useDisplay(g, 40);
  g.textAlign(g.LEFT, g.TOP);
  g.text('UNION HALL', u.x + 12, u.y + 8);
  ...
  g.noStroke();
  g.fill(C.paper); g.rect(u.x + 88, u.y + 48, 160, 128, 3);
  g.fill(C.line); g.rect(u.x + 88, u.y + 48, 160, 14, 3, 3, 0, 0);
  useDisplay(g, 30); g.fill(C.inkSoft); g.text('BULLETIN', u.x + 98, u.y + 50);
  // pinned paper slips on the hall bulletin (decorative — WARN slips are the actor)
  g.fill(C.panel);
  for (let i = 0; i < 4; i++) {
    const px = u.x + 90 + (i % 2) * 62;
    const py = u.y + 68 + Math.floor(i / 2) * 40;
    g.rect(px, py, 54, 34, 2);
    g.fill(C.inkSoft); g.rect(px + 6, py + 10, 42, 2.5); g.rect(px + 6, py + 18, 32, 2.5);
    g.fill(C.lamp); g.ellipse(px + 27, py + 5, 6, 6);
    g.fill(C.panel);
  }
  ...
  g.fill(C.ink); useDisplay(g, 40); g.text('FILM OFFICE', f.x + 12, f.y + 6);
  // brass nameplate rail under the lettering
  g.fill(C.stuccoDk); g.rect(f.x + 14, f.y + 32, 118, 3);
  g.fill(mixHex(C.stucco, C.windowWarm, 0.35)); g.rect(f.x + 14, f.y + 32, 118, 1);
  ...
  g.noStroke();
  // quiet entry canopy + door (static dressing)
  g.fill(C.tower);
  g.quad(f.x + 12, f.y + f.h - 62, f.x + 76, f.y + f.h - 62, f.x + 80, f.y + f.h - 46, f.x + 8, f.y + f.h - 46);
  g.fill(mixHex(C.paperDark, C.ink, 0.2));
  g.rect(f.x + 20, f.y + f.h - 46, 42, 46);
  g.fill(255, 250, 240, 40); g.rect(f.x + 20, f.y + f.h - 46, 5, 46);
  g.fill(C.inkSoft); g.ellipse(f.x + 52, f.y + f.h - 24, 6, 6);
  // flagpole
  ...
  g.fill(C.ink); useDisplay(g, 34); g.text('COFFEE', cc.x + 2, cc.y + 34);
  // cups on the ledge
  ...
  // sidewalk planters (decorative — not data)
  planter(g, A.sidewalk.x + 24, A.sidewalk.y + 8);
  planter(g, A.sidewalk.x + A.sidewalk.w / 2 - 26, A.sidewalk.y + 8);
  planter(g, A.sidewalk.x + A.sidewalk.w - 56, A.sidewalk.y + 8);
  plaque(g, D.street.x + 16, D.street.y + 6, 'BACKLOT STREET');
  ...
  // soffit under the gate roof
  g.fill(C.roofDk); g.rect(gm.x - 18, gm.y - 64, gm.w + 36, 6);
  g.fill(C.roof); g.rect(gm.x - 18, gm.y - 78, gm.w + 36, 18, 2);
  // ridge cap + hanging lamp housings (decorative — lamps-as-data live on stages)
  g.fill(C.roofDk); g.rect(gm.x - 14, gm.y - 82, gm.w + 28, 5, 1);
  g.fill(C.lampOff);
  g.ellipse(gm.x + 22, gm.y - 56, 34, 34);
  g.ellipse(gm.x + gm.w - 22, gm.y - 56, 34, 34);
  g.fill(mixHex(C.lampOff, C.ink, 0.15));
  ...
  // departures LED rows (blank — text from departures actor)
  g.fill(43, 105, 99, 40);
  ...
  plaque(g, D.gate.x + 16, D.gate.y + 6, 'THE GATE');
  ...
  g.fill(C.paper); useDisplay(g, 42); g.textAlign(g.LEFT, g.TOP);
  g.text('LOT CINEMA', th.x + 16, th.y + 80);
  ...
  g.fill(C.ink); useDisplay(g, 38); g.text('NEWS', ns.x + 14, ns.y + ns.h - 32);
  ...
  plaque(g, D.boulevard.x + 16, D.boulevard.y + 6, 'THE BOULEVARD');

  // California boulevard palms (decorative landscaping — not data)
  palm(g, A.streetRun.x + 80, A.streetRun.y + 20, 1.7, night);
  palm(g, A.streetRun.x + 220, A.streetRun.y + 28, 1.5, night);
  palm(g, A.theater.x - 28, A.theater.y + A.theater.h - 8, 1.65, night);
  palm(g, A.theater.x + A.theater.w + 36, A.theater.y + A.theater.h - 10, 1.85, night);
  palm(g, A.newsstand.x - 40, A.newsstand.y + A.newsstand.h, 1.4, night);
  // decorative boulevard street lamps (architecture — not FilmLA stage lamps)
  streetLamp(g, A.streetRun.x + 140, A.streetRun.y + 26, night);
  streetLamp(g, A.streetRun.x + 320, A.streetRun.y + 26, night);
  streetLamp(g, A.streetRun.x + 500, A.streetRun.y + 26, night);
  streetLamp(g, A.theater.x + A.theater.w + 80, A.theater.y + A.theater.h - 4, night);
  ...
  plaque(g, D.block.x + 16, D.block.y + 6, 'THE BLOCK');
  // quiet street trees along the block (secondary landscaping)
  tree(g, ap.x - 28, ap.y + ap.h - 10, 30, night);
  tree(g, ap.x + ap.w + 20, ap.y + ap.h - 6, 34, night);
  ...
  plaque(g, D.park.x + 16, D.park.y + 6, 'THE PARK');
...
function drawPark(g, matGrass, grassHex, night) {
  ...
  g.fill(C.curb);
  g.beginShape();
  g.vertex(px + 40, py + 20);
  g.bezierVertex(px + 120, py + 80, px + 80, py + 200, px + 160, py + 280);
  g.bezierVertex(px + 240, py + 360, px + 180, py + 460, px + 260, py + ph - 30);
  g.bezierVertex(px + 200, py + ph - 20, px + 220, py + 360, px + 140, py + 280);
  g.bezierVertex(px + 60, py + 200, px + 100, py + 80, px + 20, py + 30);
  g.endShape(g.CLOSE);
  // layered tree canopies (decorative; not data)
  tree(g, px + 70, py + 160, 58, night);
  tree(g, px + 300, py + 200, 68, night);
  tree(g, px + 190, py + 420, 50, night);
  tree(g, px + 340, py + 360, 44, night);
  tree(g, px + 120, py + 320, 42, night);
  tree(g, px + 260, py + 90, 38, night);
  tree(g, px + 50, py + 250, 36, night);
  tree(g, px + 320, py + 280, 40, night);
  // shrub berms along the lawn edge
  shrub(g, px + 40, py + ph - 40, 28, night);
  shrub(g, px + 80, py + ph - 28, 20, night);
  shrub(g, px + pw - 50, py + 60, 24, night);
  shrub(g, px + pw - 90, py + ph - 50, 22, night);
  shrub(g, px + 160, py + 70, 18, night);
  // quiet post-and-rail along the park edge (decorative)
  g.fill(C.stuccoDk);
  for (let i = 0; i < 6; i++) {
    const rx = px + 30 + i * ((pw - 60) / 5);
    g.rect(rx, py + ph - 22, 5, 20);
  }
  g.rect(px + 30, py + ph - 14, pw - 60, 4);
  // static bench (lot dressing — benches actor may add data-driven ones separately)
  g.fill(26, 32, 34, 30); g.ellipse(px + 210, py + 250, 78, 16);
  g.fill(C.stuccoDk); g.rect(px + 172, py + 228, 76, 10, 1);
  g.rect(px + 178, py + 238, 8, 18); g.rect(px + 230, py + 238, 8, 18);
  g.fill(C.stucco); g.rect(px + 174, py + 218, 72, 9, 1); // backrest
  // quiet birdbath on the lawn (decorative — not data)
  g.fill(26, 32, 34, 28); g.ellipse(px + 155, py + 175, 46, 14);
  g.fill(C.curb); g.ellipse(px + 150, py + 164, 44, 16);
  g.fill(C.water); g.ellipse(px + 150, py + 162, 32, 10);
  g.fill(C.stuccoDk); g.rect(px + 143, py + 166, 14, 22);
  g.fill(C.curb); g.ellipse(px + 150, py + 188, 22, 9);
  // picnic table (decorative — not data)
  g.fill(26, 32, 34, 26); g.ellipse(px + 280, py + 300, 84, 18);
  g.fill(C.stuccoDk); g.rect(px + 242, py + 278, 76, 12, 1);
  g.rect(px + 252, py + 290, 8, 20); g.rect(px + 300, py + 290, 8, 20);
  g.rect(px + 240, py + 266, 80, 9, 1);
  // path edge stones
  g.fill(C.stuccoDk);
  for (let i = 0; i < 10; i++) {
    const t = i / 9;
    const sx = px + 48 + t * 190;
    const sy = py + 36 + t * 230;
    g.ellipse(sx, sy, 12, 7);
  }
  // quiet path lamp near the lawn bench (decorative — not FilmLA stage lamps)
  streetLamp(g, px + 230, py + 248, night);
  streetLamp(g, px + 90, py + 140, night);
  // lawn stepping stones near the birdbath
  g.fill(C.curb);
  for (const [ox, oy] of [[130, 190], [148, 205], [168, 218], [188, 228]]) {
    g.ellipse(px + ox, py + oy, 16, 9);
  }
  // drinking fountain (decorative — not data)
  g.fill(26, 32, 34, 24); g.ellipse(px + 95, py + 240, 28, 10);
  g.fill(C.metal); g.rect(px + 88, py + 220, 14, 22, 1);
  g.fill(C.metalDk); g.rect(px + 90, py + 214, 10, 8, 1);
  g.fill(C.water); g.ellipse(px + 95, py + 216, 8, 5);
  const k = A.kiosk;
  contactShadow(g, k.x, k.y, k.w, k.h);
  g.fill(C.paperDark); g.rect(k.x, k.y, k.w, k.h, 6);
  g.fill(C.tower); g.rect(k.x, k.y - 18, k.w, 18, 3);
  g.fill(C.windowWarm); g.rect(k.x + 8, k.y + 12, k.w - 16, 36, 2);
  g.stroke(C.paperDark); g.strokeWeight(1.4);
  g.line(k.x + k.w / 2, k.y + 14, k.x + k.w / 2, k.y + 46);
  g.line(k.x + 10, k.y + 30, k.x + k.w - 10, k.y + 30);
  g.noStroke();
...
function planter(g, x, y) {
  g.fill(C.stuccoDk); g.rect(x, y + 14, 72, 36, 2);
  g.fill(C.asphaltDk); g.rect(x + 2, y + 12, 68, 12);
  g.fill(C.grassDk); g.ellipse(x + 36, y + 8, 66, 32);
  g.fill(C.grass); g.ellipse(x + 20, y + 4, 32, 20);
  g.ellipse(x + 50, y + 5, 30, 18);
  g.fill(C.lamp); g.ellipse(x + 36, y + 2, 14, 14);
  g.fill(C.lamp); g.ellipse(x + 24, y + 6, 8, 8);
}

/** Quiet boulevard lamp post — not the FilmLA stage lamps (those are actors). */
function streetLamp(g, x, y, night) {
  g.fill(26, 32, 34, 36); g.ellipse(x + 3, y + 5, 32, 12);
  g.fill(C.metalDk); g.rect(x - 7, y - 118, 14, 118);
  // base collar
  g.fill(C.metal); g.rect(x - 14, y - 10, 28, 12, 1);
  g.fill(C.metal); g.rect(x - 20, y - 126, 40, 16, 2);
  g.fill(night ? C.windowWarm : C.paper);
  g.ellipse(x, y - 142, 42, 40);
  g.fill(255, 250, 240, night ? 120 : 75);
  g.ellipse(x - 8, y - 148, 16, 14);
  if (night) {
    g.fill(241, 199, 106, 52);
    g.ellipse(x, y - 142, 120, 90);
    g.fill(241, 199, 106, 84);
    g.ellipse(x, y - 142, 62, 48);
  }
}
...
function stage(g, mat, matMetal, x, y, w, h, n, profile, night = false) {
  ...
  // bollards flanking the apron
  g.fill(C.inkSoft);
  g.rect(x + 6, y + h + 1, 22, 36, 2);
  g.rect(x + w - 40, y + h + 1, 22, 36, 2);
  g.fill(C.lamp); g.ellipse(x + 17, y + h + 1, 24, 12); g.ellipse(x + w - 29, y + h + 1, 24, 12);
  g.fill(C.paper); g.ellipse(x + 17, y + h - 2, 14, 7); g.ellipse(x + w - 29, y + h - 2, 14, 7);
  // loading-dock stairs on sawtooth stages (silhouette variation — not data)
  if (profile === 'saw') {
    g.fill(C.asphaltDk);
    for (let s = 0; s < 3; s++) {
      g.rect(x + w - 40 + s * 2, y + h + 2 + s * 5, 22 - s * 2, 5);
    }
    g.fill(C.metalDk);
    g.rect(x + w - 42, y + h - 10, 4, 28);
  }
  // quiet generator / distro box on flat stages (decorative — not a lamp count)
  if (profile === 'flat') {
    g.fill(C.metal);
    g.rect(x + 2, y + h - 56, 48, 52, 2);
    if (matMetal) stampMaterial(g, matMetal, x + 2, y + h - 56, 48, 52, 36);
    g.fill(C.metalDk);
    g.rect(x + 8, y + h - 46, 14, 12);
    g.rect(x + 28, y + h - 46, 14, 12);
    g.fill(C.lampOff); g.ellipse(x + 26, y + h - 22, 14, 14);
    g.fill(C.inkSoft); g.rect(x + 8, y + h - 12, 36, 5);
    // cable whip from the box toward the apron
    g.noFill(); g.stroke(C.inkSoft); g.strokeWeight(3.2);
    g.bezier(x + 48, y + h - 18, x + 62, y + h - 8, x + 70, y + h + 6, x + 84, y + h + 10);
    g.noStroke();
  }
  // cable tray under monitor roof (silhouette variation — not data)
  if (profile === 'monitor') {
    g.fill(C.metalDk);
    g.rect(x + 12, y + h - 10, w - 24, 7, 1);
    g.fill(C.metal);
    for (let i = 0; i < 4; i++) g.rect(x + 20 + i * ((w - 48) / 3), y + h - 18, 10, 8, 1);
  }

  // number plate
  g.fill(26, 32, 34, 30); g.ellipse(x + w / 2 + 2, y + 54, 76, 16);
  g.fill(C.panel); g.rect(x + w / 2 - 34, y + 4, 68, 54, 3);
  g.fill(255, 250, 240, 55); g.rect(x + w / 2 - 34, y + 4, 68, 6, 3, 3, 0, 0);
  g.fill(C.ink); useDisplay(g, 40); g.textAlign(g.CENTER, g.CENTER);
  g.text(String(n), x + w / 2, y + 32);
  g.textAlign(g.LEFT, g.TOP);

  // lamp housing (lamps actor lights it)
  g.fill(C.lampOff); g.ellipse(x + w / 2, y + h - 60, 32, 32);
  g.fill(mixHex(C.lampOff, C.ink, 0.18)); g.ellipse(x + w / 2, y + h - 60, 24, 24);
  g.fill(C.inkSoft); g.rect(x + w / 2 - 8, y + h - 86, 16, 22);
  // housing bracket arm + collar
  g.fill(C.metalDk);
  g.rect(x + w / 2 - 18, y + h - 74, 36, 8);
  g.rect(x + w / 2 - 12, y + h - 80, 24, 6);
}
...
function cinema(g, mat, x, y, w, h, night = false) {
  ...
  g.fill(C.lamp);
  g.rect(x - 10, y - 36, w + 20, 24);
  g.fill(C.cinemaWall);
  g.rect(x + 36, y - 58, w - 72, 26);
  g.fill(C.lamp);
  g.rect(x + 80, y - 78, w - 160, 22);
  // zig-zag deco band on the middle crown step
  g.fill(C.windowWarm);
  for (let i = 0; i < 7; i++) {
    const zx = x + 50 + i * ((w - 100) / 7);
    g.triangle(zx, y - 40, zx + 12, y - 54, zx + 24, y - 40);
  }
  g.fill(C.windowWarm);
  for (let i = 0; i < 9; i++) g.ellipse(x + 100 + i * ((w - 200) / 8), y - 68, 14, 14);
  // night crown glow (architecture — not data)
  if (night) {
    g.fill(241, 199, 106, 56);
    g.ellipse(x + w / 2, y - 50, w * 0.95, 88);
  }
...
function stageProp(g, x, y) {
  g.fill(26, 32, 34, 48); g.ellipse(x + 4, y + 34, 68, 18);
  g.fill(C.inkSoft);
  g.rect(x + 0, y - 82, 12, 116);
  g.rect(x - 24, y + 22, 58, 12);
  g.fill(C.stuccoDk); g.rect(x + 28, y - 6, 44, 34, 2);
  // sandbag weight
  g.fill(C.asphaltDk); g.ellipse(x + 12, y + 32, 42, 20);
  g.fill(C.metalDk); g.rect(x + 30, y + 4, 14, 12, 1);
  // second apple box beside the stand
  g.fill(C.stucco); g.rect(x + 36, y + 18, 28, 20, 1);
  g.fill(C.stuccoDk); g.rect(x + 38, y + 26, 24, 2);
}

/** Traffic cone — decorative apron dressing, not a counted prop. */
function cone(g, x, y) {
  g.fill(26, 32, 34, 40); g.ellipse(x + 1, y + 6, 38, 12);
  g.fill(C.lamp);
  g.triangle(x - 18, y, x + 18, y, x, y - 46);
  g.fill(C.paper);
  g.rect(x - 12, y - 22, 24, 10);
  g.fill(C.lamp);
  g.rect(x - 11, y - 1, 22, 7);
}

function cableRun(g, x, y) {
  g.noFill();
  g.stroke(C.inkSoft); g.strokeWeight(6.8);
  g.bezier(x, y, x + 34, y + 10, x + 56, y - 4, x + 92, y + 8);
  g.strokeWeight(4.8);
  g.bezier(x + 4, y + 4, x + 38, y + 12, x + 60, y, x + 90, y + 10);
  g.noStroke();
  g.fill(C.metalDk); g.rect(x - 12, y - 8, 22, 18, 1); g.rect(x + 76, y - 2, 22, 18, 1);
}

function plaque(g, x, y, s) {
  useDisplay(g, 32);
  const w = Math.max(200, s.length * 17 + 32);
  g.fill(26, 32, 34, 28);
  g.ellipse(x + w / 2 + 3, y + 52, w * 0.9, 14);
  g.fill(C.panel);
  g.stroke(C.line);
  g.strokeWeight(1.5);
  g.rect(x, y, w, 52, 3);
  g.noStroke();
  g.fill(255, 250, 240, 50);
  g.rect(x + 1, y + 1, w - 2, 6, 3, 3, 0, 0);
  g.fill(C.ink); g.textAlign(g.LEFT, g.CENTER);
  g.text(s, x + 14, y + 28);
  g.textAlign(g.LEFT, g.TOP);
}
