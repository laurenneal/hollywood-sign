...
const PREMISE = 'The sky shows the mood of Hollywood\u2019s trade press, not LA\u2019s weather.';
const MOOD = { golden: 'unusually upbeat', overcast: 'about as usual', rain: 'gloomier than usual', storm: 'much gloomier than usual' };
const SKY_NOW = { golden: 'the lot glows at golden hour', overcast: 'the sky is overcast', rain: 'it is raining', storm: 'a storm is rolling in' };
...
export default {
  ...
  legend(sig, scene) {
    if (!sig || sig.value == null) {
      return { title: 'Weather: plain sky', text: `${PREMISE} There is no reading yet, so it is a plain day.`, sig: sig || null };
    }
    const z = signedZ(sig);
    if (z == null) {
      return { title: 'Weather: plain sky', sig,
               text: `${PREMISE} There is not enough history yet to tell a good day from a bad one, so it is a plain day.` };
    }
    const state = stateOf(sig, z);
    const text =
      `${PREMISE} On ${periodLabel(sig)}, coverage in Variety, Deadline and the other trades was ${MOOD[state]}, so ${SKY_NOW[state]}.` +
      ' Upbeat coverage brings golden sun; gloomy coverage brings rain, then storms.' +
      (sig.stale ? ' There is no fresh reading, so the sky is shown grey.' : '');
    return { title: `Weather: ${STATES[state].name}`, text, sig };
  },
};
