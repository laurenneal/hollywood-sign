...
const DEFAULT_WINDOW = 24;                       // feeds/ca_credit_rounds.manifest.json "window"; sig.window_n wins
...
export default {
  ...
  draw(p, t, scene) {
    ...
    // the wall sign under the window
    ...
    p.fill(inkCol); useDisplay(p, 24); p.textAlign(p.CENTER, p.TOP);
    if (this.sign) {
      if (this.sign.line2) { p.text(this.sign.line1, sgn.x + sgn.w / 2, sgn.y + 6); p.text(this.sign.line2, sgn.x + sgn.w / 2, sgn.y + 30); }
      else { p.textSize(26); p.text(this.sign.line1, sgn.x + sgn.w / 2, sgn.y + 16); }
    }
    p.textAlign(p.LEFT, p.TOP);

    // the window and the sign explain themselves on hover (the region stops above the coffee-cart awning)
  ...
  legend(sig, scene) {
    if (!sig || sig.value == null) {
      return {
        title: 'The film office',
        text: 'No CA tax-credit round loaded — empty counter.',
        source: sig ? provenance(sig) : 'no signal loaded',
      };
    }
    const n = this.n ?? population(sig, MIN, MAX, null);
    const win = sig.window_n || DEFAULT_WINDOW;
    const indie = this.indie != null ? `${Math.round(this.indie * 100)}% independent` : 'indie share n/a';
    const year = sig.label ? `, program year ${sig.label}` : '';
    const days = this.days != null ? `${Math.round(this.days).toLocaleString('en-US')} CA filming days` : 'filming days n/a';
    const credits = this.credits != null ? `${money(this.credits)} in credits` : 'credits n/a';
    const ago = this.daysAgo == null ? '' : this.daysAgo === 0 ? ' Allocated today.' : ` Allocated ${this.daysAgo} day${this.daysAgo === 1 ? '' : 's'} ago.`;
    const stamp = this.rateFrom === 'days' ? ` Clerk stamps ${this.rate.toFixed(1)}/s from filming-days percentile.`
                : this.rateFrom === 'projects' ? ` Clerk stamps ${this.rate.toFixed(1)}/s from project percentile (no filming days).`
                : '';
    return {
      title: `${n} sheets on the counter`,
      text: `${fmt(Math.round(sig.value))} projects approved ${periodLabel(sig)}${year} (${indie}); ${days}; ${credits} (California Film Commission).` +
            ` ${pct(sig.normalized)} of the last ${win} rounds → 0–40 sheets.${ago}${stamp}`,
