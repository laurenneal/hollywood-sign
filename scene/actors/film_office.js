...
// under the window (bottom f.y + 120); the sign's left edge sits past the coffee-cart canopy lot.js paints out to
// cc.x + cc.w + 16, so the cart never covers it
const SIGN_DY = 116, SIGN_H = 58, SIGN_CLEAR = 22;
...
export default {
  ...
  draw(p, t, scene) {
    ...
    // the wall sign under the window's right half — CLOSE only (LAST ROUND copy is noise at district)
    if (scene.detailTier === TIER.CLOSE) {
      const f = scene.ANCHORS.filmOffice, cc = scene.ANCHORS.coffeeCart;
      const sx = cc.x + cc.w + SIGN_CLEAR;
      const sgn = { x: sx, y: f.y + SIGN_DY, w: f.x + f.w - 6 - sx, h: SIGN_H };
      ...
      const right = Math.max(w.x + w.w, sgn.x + sgn.w) + 4;
      scene.hit(w.x - 4, w.y - 4, right - (w.x - 4), (sgn.y + sgn.h + 2) - (w.y - 4), this);
    } else {
