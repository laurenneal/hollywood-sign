...
export default {
  ...
  legend(sig) {
    if (this.mode === 'trade' || this.mode === 'stale') {
      ...
      const n = Math.round((sig && sig.value != null) ? sig.value : this.lines.length);
      const staleBit = this.mode === 'stale' ? ' — last-known crawl, held while the feed is stale' : '';
      return {
        title: 'Trade billboard',
        text: `${n} short GDELT trade-domain line${n === 1 ? '' : 's'} from the last day` +
              (sig && sig.period ? ` (${periodLabel(sig)})` : '') + `${staleBit}.` +
    ...
    if (this.mode === 'house') {
      return {
        ...
        text: 'No GDELT trade crawl lines in the last day, so the boards show house notices about the lot.' +
              ' They fill with real trade headlines (Variety, Deadline, THR, IndieWire, TheWrap) when the wire moves.',
        source: sig ? provenance(sig) : 'GDELT Project (gdeltproject.org)',
      };
    }
    return {
      title: 'Trade billboard — dark',
      text: 'The trade feed is unavailable right now, so the boards are dark.' +
            ' They carry short GDELT trade-domain crawl lines when the feed returns.',
      source: sig ? provenance(sig) : 'GDELT Project (gdeltproject.org)',
    };
  },
};
