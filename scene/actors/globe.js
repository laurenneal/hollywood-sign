...
// GLOBAL on its face and stands a globe on a ring stand on the roof, above the Sundance queue rather than
// in it. The globe spins at 0.2 + 1.8 x the month's percentile inside its trailing 24 months (0.2 to
// 2 rad/s), an orthographic sphere tipped 20 degrees toward the viewer with meridians, parallels and a
// fictional set of land blobs. The kiosk's counter window prints the month's number with an arrow beside
// it: up when z > 0.5, down when z < -0.5, flat between (the actor has no history, so this stands in for
// year on year). Stale: gray and still.
...
const R = 30;                            // globe radius; about half the kiosk's width
const ROOF_H = 18;                       // lot.js drawPark(): the kiosk's teal roof strip above k.y
const STEM = 10;                         // pedestal between the roof and the bottom of the sphere
...
export default {
  ...
  draw(p, t, scene) {
    ...
    // with the trend arrow in front of it
    ...
    p.text('GLOBAL', k.x + k.w / 2, k.y + 18);
    p.textStyle(p.NORMAL); p.textSize(24); p.fill(C.inkSoft);
    p.text('BOX OFFICE', k.x + k.w / 2, k.y + 38);
    const label = live ? money('$', sig.value, sig.unit) : 'no data';
    const ARROW_W = 20, GAP = 5, room = k.w - 24;
    p.textSize(26);
    const tw = p.textWidth(label);
    if (tw + ARROW_W + GAP > room) p.textSize(26 * (room - ARROW_W - GAP) / tw);
    const lw = Math.min(tw, room - ARROW_W - GAP);
    const left = k.x + k.w / 2 - (ARROW_W + GAP + lw) / 2;
    p.fill(live ? scene.paint(sig, C.ink) : C.stale); p.textAlign(p.LEFT, p.CENTER);
    p.text(label, left + ARROW_W + GAP, k.y + 70);
    p.textAlign(p.LEFT, p.TOP);
    arrow(p, left + ARROW_W / 2, k.y + 71, this.trend, paint, C);

    // stand on the roof: foot, stem, then the ring on the right side from pivot to pivot
    const roofY = k.y - ROOF_H;
    p.fill(C.inkSoft); p.ellipse(cx, roofY, 30, 7); p.rect(cx - 3, cy + R + 2, 6, roofY - (cy + R + 2), 2);
    // pedestal plate under the sphere
    p.fill(paint(C.tower)); p.ellipse(cx, cy + R + 4, 16, 5);
    ...
    // hover: the globe on its stand and the kiosk under it
    scene.hit(k.x - 6, cy - R - 10, k.w + 12, (k.y + k.h + 6) - (cy - R - 10), this);
  },

  legend(sig) {
    if (!sig || sig.value == null) {
      return { title: 'Worldwide box office', text: 'No worldwide box-office figure yet, so the globe stands still.', sig: sig || null };
    }
    return {
      title: 'Worldwide box office',
      text: `Movies took ${money('$', sig.value, sig.unit)} at cinemas worldwide in ${plainPeriod(sig)}${dashVersus(sig)}.` +
            (sig.stale ? ' There has been no update since, so the globe stands still.'
              : ' The globe spins faster when the month is bigger; the arrow shows which way it is heading.'),
      sig,
    };
  },
};

// the trend arrow centred on (x, y): up in green, down in red, flat in ink; a dash when there is no data
function arrow(p, x, y, trend, paint, C) {
  p.noStroke();
  if (trend === 'up') {
    p.fill(paint(C.poster[5])); p.triangle(x - 8, y - 1, x + 8, y - 1, x, y - 10); p.rect(x - 2.5, y - 2, 5, 11);
  } else if (trend === 'down') {
    p.fill(paint(C.lamp)); p.triangle(x - 8, y + 1, x + 8, y + 1, x, y + 10); p.rect(x - 2.5, y - 9, 5, 11);
  } else if (trend === 'flat') {
    p.fill(paint(C.inkSoft)); p.rect(x - 10, y - 2, 12, 4); p.triangle(x + 2, y - 7, x + 2, y + 7, x + 10, y);
  } else {
    p.fill(C.stale); p.rect(x - 7, y - 1.5, 14, 3);
  }
}
