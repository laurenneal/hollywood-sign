...
export default {
  ...
  legend(sigs, scene, extra) {
    ...
    const title = `${lane.name} plane (${lane.tag})`;
    if (!sig) {
      return { title: `${title}: grounded`,
               text: `${k === 'ldn' ? 'bfi_inward_spend' : 'georgia_productions'} is not in signals.json yet, so the plane waits on the hill. Context, not in the index.` };
    }
    if (sig.value == null) {
      return { title: `${title}: grounded`, text: `The ${lane.name} signal has no value yet, so the plane waits on the hill. Context, not in the index.`, source: provenance(sig) };
    }
    const size = this.size[k] ?? population(sig, MIN, MAX, null);
    const win = sig.window_n || lane.window;
    const rule = ` Wingspan = 40 + 50 × that percentile. Region context only — not a relocation flow.`;
    if (k === 'ldn') {
      return {
        title: `${title}, ${size} px wingspan`,
        text: `${size} px = UK inward film and high-end TV spend ${money('£', sig.value, sig.unit)} in the rolling year to ${periodLabel(sig)} (BFI), ${pct(sig.normalized)} of the trailing ${win} ${cadenceUnit(sig.cadence, lane.unit)}.${rule} Context, not in the index.`,
        source: provenance(sig),
      };
    }
    const spend = scene && typeof scene.sig === 'function' ? scene.sig(SPEND_ID) : null;
    const spendText = spend && spend.value != null ? `, ${money('$', spend.value, spend.unit)} spend` : '';
    return {
      title: `${title}, ${size} px wingspan`,
      text: `${size} px = ${fmt(sig.value)} productions in Georgia in fiscal ${fiscalLabel(sig)} (Georgia Film Office)${spendText}, ${pct(sig.normalized)} of the trailing ${win} ${cadenceUnit(sig.cadence, lane.unit)}.${rule} Context, not in the index.`,
      source: provenance(sig),
    };
  },
};
...
function cadenceUnit(c, fallback) {
  return { hourly: 'hours', daily: 'days', weekly: 'weeks', monthly: 'months', quarterly: 'quarters', annual: 'years', yearly: 'years' }[c] || fallback || 'periods';
}
