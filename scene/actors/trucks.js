...
import { inversePopulation, pct, periodLabel, provenance } from '../scale.js';
...
export default {
  ...
  draw(p, t, scene) {
    ...
    // stencil on the gate wall, right of the gate mouth (panel plate for contrast on asphalt)
  ...
  legend(sig) {
    if (!sig || sig.value == null) {
      return { title: 'Trucks at the gate', text: 'Employment (Income domain): no LA jobs share loaded — empty lane.', source: provenance(sig) };
    }
    const n = this.n ?? inversePopulation(sig, MIN, MAX, null);
    const share = shareOf(sig);
    const ratio = sig.baseline_ratio != null ? `; ${Math.round(sig.baseline_ratio * 100)}% of the 2019 share` : '';
    return {
      title: `${n} truck${n === 1 ? '' : 's'} at the gate`,
      text: `Employment (Income domain; jobs share is the proxy): LA holds ${share}% of US film & TV jobs, ${periodLabel(sig)}` +
            ` (${pct(sig.normalized)} of the last ${sig.window_n || 24} months).` +
            ` Queue = 0–${MAX} trucks, longer as the share falls${ratio}.` +
            ` Context only — not a count of trucks leaving for other regions.`,
