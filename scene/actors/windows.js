...
const MARGIN_X = 20, MARGIN_Y = 22;      // inside the apartment rect
...
export default {
  ...
  update(sig, scene) {
    ...
    const cellH = (ap.h - MARGIN_Y * 2) / ROWS;
    this.windows = [];
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        this.windows.push({
          x: ap.x + MARGIN_X + c * cellW + (cellW - WIN_W) / 2,
          y: ap.y + MARGIN_Y + r * cellH + (cellH - WIN_H) / 2,
  ...
  draw(p, t, scene) {
    ...
    scene.hit(ap.x, ap.y - 10, ap.w, ap.h + 40, this);
  ...
  legend(sig, scene, extra) {
    ...
    if (!sig || sig.value == null || this.nBlue == null) {
      return { title: 'The apartment block', text: `No windows are lit: ${noData(sig)}.`, source: sig ? provenance(sig) : 'netflix_hours: not in signals.json yet' };
    }
    const ent = this.entrants;
    const flick = ent && ent.value != null
      ? `; ${this.nFlicker} flicker = new entrants`
      : '; flicker waits on netflix_new_entrants';
    const sub = this.share != null ? `; ${Math.round(this.share * 100)}% non-English` : '';
    return {
      title: `${this.nBlue} blue windows`,
      text: `${hoursM(sig)}M hours on Netflix’s global Top 10, week ending ${periodLabel(sig)}` +
            ` (${pct(sig.normalized)} of ${sig.window_n || 52} weeks)${flick}${sub}.` +
            ` Context only — derived aggregates, not in the index. 240 × percentile as blue windows (blinds by day).`,
  ...
  towerLegend(scene) {
    ...
    if (!epi || epi.value == null) {
      return { title: 'Broadcast tower', text: 'Blinks once a second until today’s episode count lands; then faster with that day’s percentile.',
               source: epi ? provenance(epi) : 'tv_episodes_today: not loaded' };
    }
    const rate = 1 + 3 * norm(epi, 0);
    return {
      title: 'Broadcast tower',
      text: `${rate.toFixed(1)} Hz = ${fmt(epi.value)} TV episodes airing ${periodLabel(epi)}` +
            ` (${pct(epi.normalized)} of the trailing window). Rate = 1 + 3 × percentile.`,
      source: provenance(epi),
    };
  },
};
...
function noData(sig) {
  if (!sig) return 'netflix_hours not in signals.json';
  if (sig.value == null) return 'netflix_hours has no value yet';
  return 'netflix_hours has no percentile yet (not enough weeks of history)';
}
