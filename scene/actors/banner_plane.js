...
export default {
  ...
  legend(sig) {
    const rule = `The plane flies when ${THRESHOLD} or more trade stories in a day share a big theme: strikes, layoffs, shutdowns, greenlights, acquisitions or festivals.`;
    if (this.flag) {
      const f = this.flag;
      return {
        title: `Banner plane: ${this.word}`,
        text: `${fmt(f.count)} trade stories in the last day mentioned \u201c${f.word}\u201d, the biggest theme, so the plane tows it. ${rule}`,
        sig: f.sig,
      };
    }
    if (!sig || sig.value == null) {
      return { title: 'Banner plane grounded', text: `No trade-press stories loaded yet. ${rule}`, sig: sig || null };
    }
    const best = this.largest ? ` The most common today was \u201c${this.largest.word}\u201d, in ${fmt(this.largest.count)}.` : '';
    return { title: 'Banner plane grounded', text: `No theme reached ${THRESHOLD} stories today.${best} ${rule}`, sig };
  },
};
