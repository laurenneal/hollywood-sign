...
const PAD_X = 18;
// Plain board names by signal-id prefix; anything unlisted falls back to its (shortened) title.
const NAMES = [
  ['filmla_', 'LA shoot days'], ['nyc_permits', 'NYC film permits'], ['warn_', 'Layoff notices'],
  ['weekend_no1', 'Box office #1'], ['netflix_', 'Netflix Top 10'], ['unemp_512', 'Film & TV unemployment'],
  ['us_jobs', 'US film & TV jobs'], ['global_box_office', 'Worldwide box office'], ['edd_la_mp_jobs', 'LA film & TV jobs (state count)'],
  ['la_jobs', 'LA film & TV jobs'], ['la_share', 'LA share of US jobs'], ['la_qcew', 'LA County jobs census'],
  ['bfi_', 'UK production'], ['studio_revenue', 'Studio revenue'], ['ny_credit', 'New York tax credit'],
  ['ca_credit', 'California tax credit'], ['georgia_', 'Georgia productions'], ['sundance_', 'Sundance'],
];

function boardName(group) {
  const id = group.ids[0] || '';
  const hit = NAMES.find(([prefix]) => id.startsWith(prefix));
  return hit ? hit[1] : shorten(group.title, TITLE_CHARS);
}
...
export default {
  ...
  draw(p, t, scene) {
    ...
    p.noStroke();
    if (overview) {
      // Compact schedule card on the small board silhouette lot.js paints below close zoom (dep.x + 4, 228 x 96)
      const cardW = 200, cardH = 72;
      const cx = b.x + 14, cy = b.y + 14;
      if (scene.isNight) { p.fill(amber + '1c'); p.rect(b.x - 4, b.y - 4, 244, 112, 8); }
      p.fill(26, 32, 34, 200);
      ...
      scene.hit(b.x + 4, b.y + 4, 228, 96, this);
      return;
    }
    ...
    if (!this.rows.length) {
      ...
      for (let i = 0; i < this.rows.length; i++) {
        ...
        p.fill(col);
        p.text(fitText(p, `${boardDate(r.date)}  ${r.name}`, innerW), b.x + PAD_X, y);
      }
    }
  ...
  legend() {
    const lead = 'When each number on the lot updates next, soonest first. Most move monthly or quarterly, so the lot changes slowly.';
    if (!this.rows.length) {
      return { title: 'Next data', text: `${lead} Nothing is scheduled right now.`, sig: null };
    }
    const next = this.rows.map(r => `${r.name}, ${shortDate(r.date)} (${whenWord(r.days)})`).join('; ');
    return {
      title: 'Next data',
      text: `${lead} Coming up: ${next}. Dates are estimates from each source\u2019s usual schedule.`,
      sig: null,
    };
  },
};
...
function shortDate(s) {
  ...
  return new Date(Date.UTC(d.y, d.m - 1, d.d)).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' });
}
