...
export default {
  ...
  draw(p, t, scene) {
    ...
    p.noStroke();
    if (scene.isNight) { p.fill(amber + '1c'); p.rect(b.x - 8, b.y - 8, b.w + 16, b.h + 16, 10); }   // the board glows after dark (gray when stale)

    // housing: a thin inset frame on the dark board lot.js already painted
    ...
    if (this.more > 0) {                                        // overflow note in the header, clear of the six rows
    ...
    if (!this.rows.length) {
      ...
      const on = (t % 1.2) < 0.75;
      p.textFont('monospace'); p.textSize(22); p.textAlign(p.LEFT, p.TOP);
