...
import { population, norm, fmt, pct, periodLabel, provenance } from '../scale.js';
...
export default {
  ...
  legend(sig, scene) {
    if (!sig || sig.value == null) {
      return { title: 'NYC skyline',
               text: `No NYC permit number loaded, so the towers stand dark (${sig ? 'nyc_permits has no value yet' : 'nyc_permits is not in signals.json yet'}).` +
                     ' Windows light once the weekly count lands; the dataset runs about three months behind.',
               source: sig ? provenance(sig) : 'nyc_permits: not in signals.json yet' };
    }
    const n = this.n;
    const win = sig.window_n || 104;
    const trailing = win >= 52 ? `${(win / 52) % 1 ? (win / 52).toFixed(1) : win / 52} years` : `${win} weeks`;
    const sc = scene && typeof scene.sig === 'function' ? scene.sig.bind(scene) : () => null;
    const tv = sc('nyc_permits_tv') || this.tv, film = sc('nyc_permits_film') || this.film, credit = sc('ny_credit_projects') || this.credit;
    const parts = [];
    if (tv && tv.value != null) parts.push(`${fmt(tv.value)} television`);
    if (film && film.value != null) parts.push(`${fmt(film.value)} film`);
    const subset = parts.length ? ` Of them: ${parts.join(', ')}.` : '';
    // no feed defines ny_credit_projects yet, so it is reported under its own title and unit, nothing more
    const ny = credit && credit.value != null
      ? ` ${credit.title || 'ny_credit_projects'}: ${fmt(credit.value, credit.unit || '')}${credit.period ? ', ' + periodLabel(credit) : ''}.` : '';
    const head = n == null ? 'NYC skyline, dark' : `${n} lit window${n === 1 ? '' : 's'}`;
    const text = (n == null ? 'No percentile yet, so no windows are lit. ' : '= ') +
      `${fmt(sig.value)} film permit events in NYC (every category and event type) in the week ending ${periodLabel(sig)}` +
      ` (NYC Open Data; the dataset runs about 3 months behind), ${pct(sig.normalized)} of the trailing ${trailing}.` +
      subset + ny + ` Windows are lit at 60 x that percentile.`;
    return { title: head, text, source: provenance(sig) };
  },
};
