...
export default {
  ...
  draw(p, t, scene) {
    ...
    // the countdown sign on the hall wall — CLOSE only (illegible / noisy when small)
    if (this.days && this.days.value != null && scene.detailTier === TIER.CLOSE) {
      const sx = u.x + 10, sy = u.y + 34, w = 200, h = 96;
      const d = Math.round(this.days.value);
      p.noStroke();
      p.fill(scene.paint(this.days, C.ink)); p.rect(sx, sy, w, h, 3);
      p.fill(255, 250, 240, 40); p.rect(sx, sy, w, 5, 3, 3, 0, 0);
      p.fill(scene.paint(this.days, scene.isNight ? C.windowWarm : C.paper));
      useDisplay(p); p.textAlign(p.CENTER, p.TOP);
      p.textSize(24); p.text('NEXT CONTRACT:', sx + w / 2, sy + 12);
      p.textSize(42); p.text(d < 0 ? 'EXPIRED' : `${d} DAYS`, sx + w / 2, sy + 44);
      scene.hit(sx, sy, w, h, this, 'countdown');
    } else if (this.days && this.days.value != null) {
      // Compact mark so the countdown stays hittable at district/overview
      const sx = u.x + 18, sy = u.y + 42;
      p.noStroke();
      p.fill(scene.paint(this.days, C.ink));
      p.rect(sx, sy, 56, 28, 3);
      scene.hit(sx, sy, 56, 28, this, 'countdown');
    }
  ...
  legend(sig, scene, extra) {
    ...
    if (extra === 'countdown' && dtxt) {
      return {
        title: 'Next contract',
        text: `${dtxt[0].toUpperCase() + dtxt.slice(1)}${days.label ? ': ' + days.label : ''}` +
              `${days.period ? `, as of ${periodLabel(days)}` : ''}.`,
        source: provenance(days),
      };
    }
    if (!sig || sig.value == null) {
      return {
        title: 'The picket line',
        text: 'No strike flag loaded — sidewalk stays clear.',
        source: provenance(sig),
      };
    }
    const n = Math.max(0, Math.round(sig.value));
    if (n === 0) {
      return { title: 'No picket line', text: `No strike today.${dtxt ? ' ' + dtxt[0].toUpperCase() + dtxt.slice(1) + '.' : ''}`, source: provenance(sig) };
    }
    const files = Math.min(n, MAX_FILES);
    return {
      title: `${files * PER_UNION} on the picket line`,
      text: `${n} union${n === 1 ? '' : 's'} on strike, ${periodLabel(sig)}. ` +
            `${PER_UNION} walkers per union${n > MAX_FILES ? ' (sidewalk holds two files)' : ''}.` +
            (dtxt ? ` ${dtxt[0].toUpperCase() + dtxt.slice(1)}.` : ''),
      source: provenance(sig),
    };
  },
};
