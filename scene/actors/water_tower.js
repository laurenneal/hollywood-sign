...
export default {
  ...
  draw(p, t, scene) {
    ...
    // label plate under the feet — CLOSE only
    if (scene.detailTier === TIER.CLOSE) {
      p.fill(scene.paint(sig, C.paper)); p.rect(cx - 104, feetY + 12, 208, 48, 3);
      p.fill(255, 250, 240, 50); p.rect(cx - 104, feetY + 12, 208, 5, 3, 3, 0, 0);
      p.fill(scene.paint(sig, C.inkSoft)); useDisplay(p, 36); p.textAlign(p.CENTER, p.CENTER);
      p.text('SIGN INDEX', cx, feetY + 36);
    }

    p.pop();
    scene.hit(cx - 104, tankTop - ROOF_H - 16, 208, feetY + 60 - (tankTop - ROOF_H - 16), this);
  },

  legend(sig, scene) {
    if (!sig) {
      return { title: 'SIGN index', text: 'Index not built yet — empty tank.' };
    }
    if (sig.value == null) {
      return { title: 'SIGN index: not enough history yet',
               text: 'The composite needs a trailing window for every live component before it prints a number, so the tank stays empty and the needle rests at zero.' +
                     (sig.label && sig.label !== 'not enough history yet' ? ` Feed label: ${sig.label}.` : ''),
               source: provenance(sig) };
    }
    const vs = scene && typeof scene.sig === 'function' ? scene.sig('sign_index_vs2019') : null;
    const window = sig.normalized != null && sig.window_n
      ? ` (${pct(sig.normalized)} of the trailing ${sig.window_n} ${cadenceUnit(sig.cadence)})` : '';
    const text =
      `${fmt(sig.value)} of 100, ${periodLabel(sig)}${sig.label ? `, ${sig.label}` : ''}${window}.` +
      ` 50 = every component at its trailing mean; each component z clipped to ±2 and weighted per METHOD.md.` +
      ` The tank fills to index/100 and the needle points at the same number.` +
      (vs && vs.value != null ? ` vs 2019 baseline: ${fmt(vs.value, vs.unit || '')}.` : '');
    return { title: `SIGN index ${Math.round(sig.value)}`, text, source: provenance(sig) };
  },
};
