...
export default {
  ...
  draw(p, t, scene) {
    ...
    if (hasData) {
      ...
      p.text(this.title, M.x + M.w / 2, M.y + 28);
      ...
      p.fill(C.staleLite);
      p.textStyle(p.BOLD); p.textSize(28);
      p.text('NO SHOW', M.x + M.w / 2, M.y + 28);
      p.textStyle(p.NORMAL); p.textSize(22);
      p.text(this.subline, M.x + M.w / 2, M.y + 56);
    }
    p.textAlign(p.LEFT, p.TOP);

    // ---- ropes and stanchions (always there; an empty line is the no-data state)
    ...
    // ---- the line: a ring of n figures shuffling toward the doors one spot per step
  ...
  legend(sig, scene) {
    if (!sig || sig.value == null) {
      return { title: 'Nobody in line', text: 'No weekend number-one film loaded — marquee stays dark.', source: provenance(sig) };
    }
    const title = sig.label ? sig.label : 'The number-one film';
    const four = scene && scene.sig ? scene.sig('weekend_no1_4wk') : null;
    const tail = four && four.value != null ? ` Four-weekend total: ${fmt(four.value, 'USD')}.` : '';
    const est = sig.preliminary ? ' (preliminary)' : '';
    return {
      title: `${this.n} in line`,
      text: `Weekend number-one film: ${title} took ${fmt(sig.value, 'USD')}${est}, weekend ending ${periodLabel(sig)}` +
            ` (${pct(sig.normalized)} of the last ${sig.window_n || 52} #1 weekends).` +
            ` Drawn as 0–${MAX} people in line.${tail}`,
