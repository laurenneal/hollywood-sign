...
const HALO_DAY = 80, HALO_NIGHT = 120;
    ...
    for (const s of this.stages) {
      if (s.lit) {
        if (stale) {
          ...
          p.fill(C.lampOff); p.ellipse(s.cx, s.lampY, 42, 42);
          p.noFill(); p.stroke(scene.paint(sig, C.lamp)); p.strokeWeight(3.5); p.ellipse(s.cx, s.lampY, 50, 50); p.noStroke();
        } else {
          ...
          p.fill(C.lamp); p.ellipse(s.cx, s.lampY, 42, 42);
          p.fill(255, 255, 255, night ? 170 : 130); p.ellipse(s.cx - 7, s.lampY - 7, 13, 13);   // glass highlight
        }
      }
      if (s.truck) truck(p, s.truck, C, (hex) => scene.paint(sig, hex), night);
      ...
      if (s.truck) scene.hit(s.cx + 28, s.y + h, 88, 44, this, { stage: s.n });
    }
