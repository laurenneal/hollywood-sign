...
const STAGES = 12;
const LAMP_DX = 24, LAMP_DY = 94;   // lot.js stage(): housing at x + w/2 + 24, y + h - 94, on the stage sign beside the number
...
export default {
  ...
  district: 'stages',
  layer: 'mid',
  stages: [],       // the 12 housings, row-major: {n, x, y, cx, lampX, lampY, lit, phase, truck|null}
  ...
  update(sig, scene) {
    ...
    for (let r = 0; r < 2; r++) for (let i = 0; i < 6; i++) {
      ...
      this.stages.push({ n: i + 1 + r * 6, x, y, cx: x + w / 2, lampX: x + w / 2 + LAMP_DX, lampY: y + h - LAMP_DY, lit: false, phase: range(rng, 0, Math.PI * 2), truck: null });
    }
    ...
    for (const s of this.stages) {
      if (s.lit) {
        if (overview) {
          ...
          p.ellipse(s.lampX, s.lampY, CORE, CORE);
        } else if (stale) {
          // dimmed to the housing color with a gray ring, so the count still reads
          p.fill(C.lampOff); p.ellipse(s.lampX, s.lampY, CORE, CORE);
          p.noFill(); p.stroke(scene.paint(sig, C.lamp)); p.strokeWeight(2.5); p.ellipse(s.lampX, s.lampY, CORE + 8, CORE + 8); p.noStroke();
        } else {
          ...
          p.fill(lr, lg, lb, a * 0.55); p.ellipse(s.lampX, s.lampY, dia, dia);
          p.fill(lr, lg, lb, a); p.ellipse(s.lampX, s.lampY, dia * 0.55, dia * 0.55);
          p.fill(C.lamp); p.ellipse(s.lampX, s.lampY, CORE, CORE);
          p.fill(255, 255, 255, night ? 170 : 130); p.ellipse(s.lampX - 4, s.lampY - 4, 6, 6);   // glass highlight
        }
      }
      ...
      if (s.truck && !overview) scene.hit(s.cx + 30, s.y + h, 66, 44, this, { stage: s.n });   // box, cab and mirror
    }
  ...
  legend(sig, scene, extra) {
    if (!sig || sig.value == null) {
      return { title: 'Stage lamps', text: 'No filming figure from FilmLA yet, so every lamp is off.', sig: sig || null };
    }
    ...
    const days = `LA had ${num(sig.value)} days of on-location filming (street and location shoots, not studio stages) in ${plainPeriod(sig)}`;
    if (!ratio) {
      return { title: 'Stage lamps', text: `${days}. It is the first quarter on record, so there is nothing to compare it with yet.`, sig };
    }
    let text = ratio.how === 'baseline'
      ? `${days}: ${Math.round(Number(sig.baseline_ratio) * 100)}% of the ${ref} pace. All 12 lamps lit would mean filming at ${ref} levels.`
      : `${days}${dashVersus(sig)}. More filming, more lamps lit.`;
    if (n > 0 && this.mix) {
      const parts = CATS.filter(c => this.mix[c.key]).map(c => `${this.mix[c.key]} ${c.name}`);
      text += ` The trucks outside show the mix: ${parts.join(', ')}.`;
    }
    if (n > 0 && this.share != null) {
      text += ` ${Math.round(this.share * 100)}% of the filming had California\u2019s tax credit; those stages fly a pennant.`;
    }
    if (extra && extra.stage) {
      const s = this.stages[extra.stage - 1];
      if (s) text += s.lit ? ` Stage ${s.n} is lit.` : ` Stage ${s.n} is dark.`;
    }
    return { title: `${n} of 12 stages lit`, text, sig };
  },
};
