...
const MIN = 0, MAX = 14;
const BENCHES = 3, SEATS = 4, BENCH_W = 168, BENCH_GAP = 22, SEAT_PITCH = 40;
const CART_QUEUE = 4, QUEUE_PITCH = 48;
const FIG = 1.25;
const DEFAULT_WINDOW = 36;
...
export default {
  ...
  update(sig, scene) {
    ...
    // three planks along the hall's front, the seat line in the strip between the wall and the sidewalk
    const seatY = u.y + u.h + 10;
    const x0 = u.x + 12;
    this.benches = [];
    ...
    const seats = [];
    for (const b of this.benches) for (let s = 0; s < SEATS; s++) seats.push({ x: b.x + 4 + s * SEAT_PITCH, y: seatY, kind: 'seat' });
    shuffle(rng, seats);
    const cart = [];
    for (let i = 0; i < CART_QUEUE; i++) cart.push({ x: cc.x + cc.w + 10 + i * QUEUE_PITCH, y: cc.y + cc.h - 4, kind: 'cart' });
    const stands = shuffle(rng, [
      { x: x0 - 9, y: seatY + 3 }, { x: x0 + BENCH_W + BENCH_GAP / 2, y: seatY + 3 },
      { x: x0 + 2 * BENCH_W + 1.5 * BENCH_GAP, y: seatY + 3 }, { x: u.x + u.w - 3, y: seatY + 3 },
      { x: u.x + u.w + 12, y: u.y + u.h - 2 }, { x: u.x + u.w + 28, y: u.y + u.h - 2 },
      { x: u.x + u.w + 12, y: u.y + u.h - 22 }, { x: u.x + u.w + 28, y: u.y + u.h - 22 },
    ].map(s => ({ ...s, kind: 'stand' })));

    // seats first, then stand / cart — 15 seats + 5 + 5 covers MAX 16 with room to spare
  ...
  draw(p, t, scene) {
    ...
    for (const q of this.people) {
      ...
      if (overview) {
        ...
        p.ellipse(q.x + 6, q.y - (q.kind === 'seat' ? 10 : 18), 12, 12);
        continue;
      }
      if (q.kind === 'seat') {
    ...
    scene.hit(u.x - 12, u.y + u.h - 10, u.w + 40, 48, this, 'benches');                  // the three benches
    scene.hit(u.x + u.w, u.y + u.h - 46, cc.x - (u.x + u.w), 56, this, 'benches');        // loiterers beside the hall
    scene.hit(cc.x + cc.w + 4, cc.y + cc.h - 48, CART_QUEUE * QUEUE_PITCH + 16, 56, this, 'cart');
  },
  ...
  legend(sig, scene, extra) {
    ...
    if (!s || s.value == null) {
      return {
        title: 'The benches',
        text: 'Employment (Income domain): no industry unemployment number yet — benches stay empty.',
        source: provenance(s),
      };
    }
    const n = this.n ?? fill(s, MIN, MAX);
    const win = s.window_n || DEFAULT_WINDOW;
    const where = extra === 'cart' ? ' Coffee-cart queue counts too.' : '';
    const kind = s.id === 'unemp_512' ? 'one month' : '12-month mean';
    return {
      title: `${n} people on the benches`,
      text: `Employment (Income domain): ${Number(s.value).toFixed(1)}% industry unemployment (${kind}), ${periodLabel(s)}` +
            ` — ${pct(s.normalized)} of the last ${win} months. Drawn as 0–${MAX} figures.${where}`,
      source: provenance(s),
    };
  },
};
