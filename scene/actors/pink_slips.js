...
export default {
  ...
  draw(p, t, scene) {
    ...
    // quiet cork speckles (deterministic grit — not data)
    p.fill(90, 70, 52, 28);
    for (let i = 0; i < 40; i++) {
    ...
    if (scene.isNight) {                                                       // a small lamp over the board
      // glow kept to the board's own width (x 1272..1388) so it never washes the lot's UNION HALL label, which ends near x 1270
