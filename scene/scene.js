...
import { provenance, indexRead } from './scale.js';
...
// Fresh load frames The Gate. Named district jumps glide; pan and zoom stay immediate.
...
//   • tap/click pins inspector; Reset returns to The Gate
...
// First visit: the intro card explains the lot over a whole-lot view; entering glides to The Gate.
function onIntroClose() {
  if (!firstRunEstablish) return;
  firstRunEstablish = false;
  if (tour) tour.go(defaultStopIndex(), { animate: true });
  startHintIfFirstRun();
}
...
function installZoomChrome(p) {
  ...
  if (!el) {
    ...
    el.innerHTML =
      ...
      '<button type="button" class="zreset" aria-label="Reset to The Gate">Reset</button>';
  ...
  el.querySelector('.zreset').onclick = () => {
    if (!useHomeView()) return;
    // Reset → The Gate (default framing)
    const i = defaultStopIndex();
...
const sketch = (p) => {
  ...
  p.setup = () => {
    ...
    // First visit shows the whole lot behind the explainer for context, then glides to The Gate on entry.
