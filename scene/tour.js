// District tour API: named stops + camera helpers. Pass 1 puts district chips in document flow at every
// width, so the old fixed bottom tour bar stays hidden (CSS + tourWanted) to avoid overlapping replay.

import { TOUR_STOPS, tourCamera, homeCamera, defaultStopIndex } from './camera.js';

function tourWanted() {
  // Overlay tour bar retired in Pass 1; #districts is the selector at all breakpoints.
  return false;
}
...
export function installTour(scene, { getP, setCamera } = {}) {
  ...
  let index = defaultStopIndex(getP && getP());
  let swipe = null;
