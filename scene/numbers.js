...
const TAGS = { live: 'up to date', stale: 'not updated', missing: 'no data yet', deferred: 'coming later' };
...
function rowState(legend, sig) {
  ...
  if ((legend && legend.deferred) || /\bdeferred\b/.test(blob)) return 'deferred';
...
export function installNumbers(scene) {
  ...
  function refresh() {
    ...
    for (const a of ACTORS) {
      ...
      const cited = Object.prototype.hasOwnProperty.call(legend, 'sig') ? legend.sig : one;
      const src = sourceHtml(cited, esc);
      rows.push(
        `<li class="${esc(state)}">` +
        `<b>${esc(legend.title)}</b>` +
        `<span class="tag">${esc(TAGS[state] || state)}</span>` +
